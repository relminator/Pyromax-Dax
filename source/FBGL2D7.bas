...
dim as GLuint current_texture = 0
dim as GLuint font_list_base = 0
dim as GLuint font_list_offset = 0
dim as GLuint font_textureID = 0
dim as GLuint glow_textureID = 0
dim as integer font_num_glyphs = 0
''*****************************************************************************
''
...
dim as single CubeVectors(23) =>_
{	(-0.5), (-0.5), ( 0.5),_ 
	( 0.5), (-0.5), ( 0.5),_
	( 0.5), (-0.5), (-0.5),_
	(-0.5), (-0.5), (-0.5),_
...
''
''*****************************************************************************
private function LoadGlowImage() as GLuint
	...
	dim as GLuint textureID = LoadImage( @image_array(0), GL_LINEAR )
...
sub FontLoad( byval glyph_width as integer, byval glyph_height as integer,_
	...
	glDeleteLists( font_list_base, font_num_glyphs )
    ...
    font_num_glyphs = TilesW * TilesH
   	
    font_list_base = glGenLists(font_num_glyphs)

    glPushMatrix()
    glLoadIdentity()
    for font_loop as integer = 0 to font_num_glyphs - 1
...
sub ScreenInit(byval screen_wid as integer, byval screen_hei as integer, byval flags as integer = 0)
	...
	if( font_list_base ) then
		glDeleteLists( font_list_base, 256 )
	endif
    if( font_textureID ) then
		glDeleteTextures( 1, @font_textureID )
	endif
    
    if( glow_textureID ) then
		glDeleteTextures( 1, @glow_textureID )
	endif
    
	'' set up the font system
	font_init()

	glow_textureID = LoadGlowImage()
...
''*****************************************************************************
''
''    Draws a clean-stretched 2d sprite 
''
''*****************************************************************************
...
''*****************************************************************************
''
''    Loads any FB.image into an OpenGL texture
''    results are stored in a glsprite datatype
''
''*****************************************************************************
...
''*****************************************************************************
''
''    use this to get time-elapsed(in seconds) between frames
''
''*****************************************************************************
function GetDeltaTime( byref FPS as double, byval TimerInSeconds as double) as double
	
	'' vars to be used in calculating FPS
	static as double FramesPerSecond = 0.0
	static as double PreviousTime = 0 
	
	'' var to save our last time for comparing 
	static as double OldTime = 0.0

	'' Get the current time in seconds
	dim as double CurrentTime = TimerInSeconds             
	
	'' Time interval between frames
	dim as double ElapsedTime = CurrentTime - OldTime
...
''
''*****************************************************************************
sub SpriteRotateScaleXY3D( byval x as integer, byval y as integer, byval z as integer, byval angle as integer, byval scaleX as single, byval scaleY as single, byval mode as GL2D_FLIP_MODE, byval spr as image ptr)
	...
	glPushMatrix()
		...
		glBegin( GL_QUADS )
	        ...
	        glTexCoord2f( u1, v1 ): glVertex3i( x1, y1,z )
	        glTexCoord2f( u1, v2 ): glVertex3i( x1, y2,z )
	        glTexCoord2f( u2, v2 ): glVertex3i( x2, y2,z )
	        glTexCoord2f( u2, v1 ): glVertex3i( x2, y1,z )
...
'' Size = TILE_SIZE
''*************************************
sub DrawFace overload( byval PolyIdx as integer, byval Size as single, byval Flipmode as integer, byval spr as IMAGE ptr )
...
'' Size = TILE_SIZE
''*************************************
sub DrawFace overload( byval PolyIdx as integer, byval Size as single, byval xoffset as single, byval yoffset as single, byval Flipmode as integer, byval spr as IMAGE ptr )
	...
	glTexCoord2f( u1 + xoffset, v1 + yoffset )
	glVertex3f(CubeVectors(f1*3) * S, CubeVectors(f1*3 + 1) * S, CubeVectors(f1*3 +  2) * S )
	
	glTexCoord2f( u2 + xoffset, v1 + yoffset )
	glVertex3f(CubeVectors(f2*3) * S, CubeVectors(f2*3 + 1) * S, CubeVectors(f2*3 + 2) * S )
	
	glTexCoord2f( u2 + xoffset, v2 + yoffset )
	glVertex3f(CubeVectors(f3*3) * S, CubeVectors(f3*3 + 1) * S, CubeVectors(f3*3 + 2) * S )

	glTexCoord2f( u1 + xoffset, v2 + yoffset )
	glVertex3f(CubeVectors(f4*3) * S, CubeVectors(f4*3 + 1) * S, CubeVectors(f4*3 + 2) * S )
...
'' Draws a 3d cube
''*************************************
sub DrawCubeFront( byval x as single, byval y as single, byval z as single, byval Size as single, byval xoffset as single, byval yoffset as single, byval spr as IMAGE ptr  )
	...
	glPushMatrix()
		glTranslatef( x + Size/2, y + Size/2, z)  '' Offset by Size/2 so that we draw from top-left at (0,0)
		glBegin( GL_QUADS )
			DrawFace( 1, Size, xoffset, yoffset, GL2D.FLIP_NONE, spr )   '' front
		glEnd()
...
'' Draws a 3d cube
''*************************************
sub DrawCubeTopFront( byval x as single, byval y as single, byval z as single, byval Size as single, byval xoffset as single, byval yoffset as single, byval spr as IMAGE ptr  )
	...
	glPushMatrix()
		glTranslatef( x + Size/2, y + Size/2, z)  '' Offset by Size/2 so that we draw from top-left at (0,0)
		glBegin( GL_QUADS )		
			DrawFace( 0, Size, xoffset, yoffset, GL2D.FLIP_NONE, spr )   '' top
			DrawFace( 1, Size, xoffset, yoffset, GL2D.FLIP_NONE, spr )   '' front
		glEnd()
...
sub DrawCubePlus( byval x as single, byval y as single, byval z as single, byval Size as single, byval xoffset as single, byval yoffset as single, byval spr as IMAGE ptr,_
	...
	glPushMatrix()
		...
		glBegin( GL_QUADS )
			...
			if( qTop    ) then DrawFace( 0, Size, xoffset, yoffset, GL2D.FLIP_V   , spr )   '' top
			if( qFront  ) then DrawFace( 1, Size, xoffset, yoffset, GL2D.FLIP_NONE, spr )   '' front
			if( qRight  ) then DrawFace( 2, Size, xoffset, yoffset, GL2D.FLIP_H   , spr )   '' right
			if( qLeft   ) then DrawFace( 4, Size, xoffset, yoffset, GL2D.FLIP_H   , spr )   '' left
			if( qBottom ) then DrawFace( 5, Size, xoffset, yoffset, GL2D.FLIP_V   , spr )   '' bottom
...
sub Box3D( byval x1 as integer, byval y1 as integer, byval x2 as integer, byval y2 as integer, byval z as integer, byval gl2dcolor as GLuint )
